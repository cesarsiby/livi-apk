...
-- Backfill des anciennes écritures de payables vendeur.
...
WHERE le.transaction_id = lt.id
  AND e.order_id = NULLIF(lt.metadata->>'order_id', '')::uuid
  AND la.id = le.account_id
  AND le.owner_user_id IS NULL
  AND la.code = 'livi_vendor_payable_xof';

-- Backfill des anciennes écritures de payables transporteur.
UPDATE ledger_entries AS le
...
FROM ledger_transactions AS lt,
     ...
     ledger_accounts AS la
WHERE le.transaction_id = lt.id
  AND s.order_id = NULLIF(lt.metadata->>'order_id', '')::uuid
  AND la.id = le.account_id
  AND le.owner_user_id IS NULL
...
FOR EACH ROW
EXECUTE FUNCTION livi_payable_requires_owner();
