...
export async function cancelOrder(c, { orderId, actorId, actorRole, reason }) {
  ...
  // A payment that is still pending at the partner cannot be cancelled locally:
  // the partner could confirm it after the cancellation request.
  ...
  if (actorRole === 'buyer' || actorRole === 'client') {
    ...
    // Vendor can cancel before shipment/pickup. Once shipped, only dispute/admin flow remains.
  ...
  if (order.status === 'pending_payment') {
    ...
    await c.query(`UPDATE orders SET status='cancelled',cancelled_at=now(),cancelled_reason=$2,updated_at=now() WHERE id=$1`,[orderId,reason]);
  ...
  await restoreStockOnce(c, orderId);
  await c.query(`UPDATE orders SET status='refunded',cancelled_at=now(),cancelled_reason=$2,updated_at=now() WHERE id=$1`,[orderId,reason]);
...
async function restoreStockOnce(c, orderId) {
  const row = (await c.query(`
    SELECT stock_restored_at FROM orders WHERE id=$1 FOR UPDATE
  `,[orderId])).rows[0];
