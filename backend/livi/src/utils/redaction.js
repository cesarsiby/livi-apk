const SENSITIVE_KEYS = new Set([
  ...
  'secret','secret_hash','code_hash','otp','otp_code','otp_hash',
  'pin','pin_hash','qr_payload','qr_secret','file_key','authorization',
  'cookie','set-cookie','rawbody','raw_body'
]);

// SESSION 26 — trouvé en lisant les vrais logs Render en production (pas
// en relisant le code), après le déploiement réel de cette session : une
// entrée bare 'code' existait ici en plus de 'code_hash'/'otp_code'/
// 'otp_hash' — mais 'code' est aussi le nom du champ métier que HttpError
// attache à CHAQUE erreur applicative (PHONE_ALREADY_REGISTERED,
// PRODUCT_SLUG_ALREADY_EXISTS, TRANSPORTER_ALREADY_HAS_ACTIVE_MISSION...),
// et celui de ledger_accounts.code / partners.code (identifiants métier,
// pas des secrets — vérifié dans migrations/001_initial.sql et
// 033_v46_architecture_alignment.sql). Conséquence observée en direct :
// chaque log d'erreur applicative affichait `code: '[REDACTED]'` au lieu
// du vrai code métier — exactement l'inverse de ce que demande le §16 du
// prompt maître ("les logs doivent contenir... le code"). Retiré : la
// vraie valeur sensible (le code OTP en clair) n'a jamais été stockée sous
// ce nom de toute façon — auth_otp_challenges ne stocke que `code_hash`
// (vérifié dans migrations/001_initial.sql), déjà couvert ci-dessus sans
// le bare 'code'. Rien de réellement secret n'est démasqué par ce retrait.
