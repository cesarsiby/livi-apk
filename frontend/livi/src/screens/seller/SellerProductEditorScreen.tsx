...
function MediaCard({
  ...
  const image = resolveMediaUrl(item.media_url ?? undefined);
      ...
      <View style={styles.mediaPreview}>
        {image ? (
          ...
          <View style={styles.mediaPlaceholder}>
            <Text style={styles.mediaPlaceholderText}>L</Text>
          </View>
        )}
      </View>
...
export function SellerProductEditorScreen({
  ...
  const [productId, setProductId] =
    useState<string | null>(routeProductId);
  ...
  const loadProduct = useCallback(async () => {
    ...
    try {
      const response = await sellerApi.products({
        limit: 100,
      });
      ...
      setPrice(
        found.price_xof != null
          ? String(found.price_xof)
          : String((found as any).price ?? ''),
      );
      setStock(
        found.stock != null
          ? String(found.stock)
      ...
      setDescription(
        String((found as any).description ?? ''),
      );
      setCategoryId(
        (found as any).category_id
          ? String((found as any).category_id)
          : null,
      );
      ...
      const [
        ...
        sellerApi.media(String(productId)),
        sellerApi.variants(String(productId)),
      ]);
  ...
  const addPhotos = async () => {
    ...
    try {
      ...
      const refreshed =
        await sellerApi.media(productId);
  ...
  const addVariant = async () => {
    ...
    try {
      ...
      const refreshed =
        await sellerApi.variants(productId);
      ...
      >
        ...
        <View style={styles.previewCard}>
          ...
          <View style={styles.previewMeta}>
            <View style={styles.previewMetaItem}>
              <Text
                style={styles.previewMetaLabel}
              >
                Stock
              </Text>
              <Text
                style={styles.previewMetaValue}
              >
                {stock || '0'}
              </Text>
            </View>
        ...
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeaderRow}>
            <View style={styles.sectionHeaderGrow}>
              <SectionHeader
                eyebrow="03"
                title="Photos"
