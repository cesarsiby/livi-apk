      ...
      {cases.map((item) => (
        <Card key={item.id} style={styles.card}>
          <View style={styles.row}>
            ...
            <Badge label={item.status} variant={CASE_STATUS_VARIANT[item.status] ?? 'gray'} />
          </View>
          <Text style={styles.body}>{item.reason}</Text>
          {item.proposed_amount != null && <Text style={styles.muted}>Montant proposé : {Number(item.proposed_amount).toLocaleString('fr-FR')} XOF</Text>}
          {item.status === 'open' && (
            <>
              <TextInput
