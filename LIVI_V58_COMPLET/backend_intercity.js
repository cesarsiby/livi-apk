...
r.post('/intercity/options', asyncHandler(async(req,res)=>{
  ...
  const products=(await pool.query('SELECT id,name,preparation_time_hours,is_perishable,shelf_life_hours,shelf_life_reference_at FROM products WHERE id=ANY($1::uuid[])',[productIds])).rows;
...
r.post('/intercity/validate', asyncHandler(async(req,res)=>{
  ...
  const products=(await pool.query('SELECT id,name,preparation_time_hours,is_perishable,shelf_life_hours,shelf_life_reference_at FROM products WHERE id=ANY($1::uuid[])',[b.items.map(i=>i.product_id)])).rows;
  const preparationHours=Math.max(...products.map(p=>Number(p.preparation_time_hours||0)),0);
...
r.post('/intercity/admin/routes', requireRoles('admin'), asyncHandler(async(req,res)=>{const b=z.object({partner_id:z.string().uuid(),origin_city:city,destination_city:city,fee_xof:z.number().int().nonnegative(),transit_min_hours:z.number().int().positive(),transit_max_hours:z.number().int().positive()}).parse(req.body);if(b.transit_max_hours<b.transit_min_hours)throw new HttpError(422,'Le délai maximum doit être supérieur ou égal au minimum');try{const row=(await pool.query(`INSERT INTO intercity_routes(partner_id,origin_city,destination_city,fee_xof,transit_min_hours,transit_max_hours,status) VALUES($1,$2,$3,$4,$5,$6,'active') RETURNING *`,[b.partner_id,b.origin_city,b.destination_city,b.fee_xof,b.transit_min_hours,b.transit_max_hours])).rows[0];ok(res,row,201);}catch(e){if(e?.code==='23505')throw new HttpError(409,'Cette liaison existe déjà pour ce partenaire','INTERCITY_ROUTE_EXISTS');throw e;}}));
