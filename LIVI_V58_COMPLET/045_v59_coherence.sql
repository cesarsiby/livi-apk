...
-- Legacy V58 minimum-guarantee field was an invented business rule. Keep the
-- column for backward schema compatibility, but neutralize its default and
-- make runtime calculations ignore it.
ALTER TABLE intercity_pricing_rules
  ALTER COLUMN minimum_guarantee_xof SET DEFAULT 0;
UPDATE intercity_pricing_rules SET minimum_guarantee_xof=0 WHERE minimum_guarantee_xof <> 0;
